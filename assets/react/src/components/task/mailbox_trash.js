...
export default class MailBoxTrash extends Component{
    constructor(props){
        super(props);
        this.state = {
            data: [],
    ...
    render(){
        ...
        if (this.state.data.total > 0 ) {
            this.state.data.data.forEach(function (data) {
                messages.push(<MailBoxItem type='trash' handleViewDetail={this.props.handleViewDetail} key={data.id} data={data}/>)
            }.bind(this));
...
MailBoxTrash.contextTypes = { lang: React.PropTypes.number,t:React.PropTypes.any};